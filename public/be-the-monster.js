...
const scene = new THREE.Scene(); scene.background = new THREE.Color(0x100707); scene.fog = new THREE.FogExp2(0x120707, 0.014);
const camera = new THREE.PerspectiveCamera(76, innerWidth / innerHeight, 0.08, 250);
const renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: 'high-performance' }); renderer.setPixelRatio(Math.min(devicePixelRatio, 1.7)); renderer.setSize(innerWidth, innerHeight); renderer.outputColorSpace = THREE.SRGBColorSpace; renderer.toneMapping = THREE.ACESFilmicToneMapping; renderer.toneMappingExposure = 1.18; root.appendChild(renderer.domElement);
const controls = new PointerLockControls(camera, renderer.domElement); scene.add(camera);
scene.add(new THREE.HemisphereLight(0x8d7672, 0x23100e, 1.5)); const moon = new THREE.DirectionalLight(0xc1aaa1, 1.7); moon.position.set(-30,45,25); scene.add(moon);
...
let started = false, paused = true, ended = false, feeding = false, feedTime = 0, dangerTimer = 0;
...
function buildMaze(){maze=generateMaze();const wallMat=new THREE.MeshStandardMaterial({color:0x5b514a,roughness:0.98});const floorMat=new THREE.MeshStandardMaterial({color:0x463c35,roughness:1});const floor=new THREE.Mesh(new THREE.PlaneGeometry(GRID*CELL,GRID*CELL),floorMat);floor.rotation.x=-Math.PI/2;scene.add(floor);for(let y=0;y<GRID;y++)for(let x=0;x<GRID;x++)if(maze[y][x]===1){const w=new THREE.Mesh(new THREE.BoxGeometry(CELL,4.5,CELL),wallMat);const p=world({x,y});w.position.set(p.x,2.25,p.z);scene.add(w);}for(let i=0;i<16;i++){const l=new THREE.PointLight(i%2?0x7f1d12:0x402018,2.2,18,2);const c={x:1+Math.floor(Math.random()*(GRID-2)),y:1+Math.floor(Math.random()*(GRID-2))};const p=world(c);l.position.set(p.x,4.2,p.z);scene.add(l);}}
...
function beginFeed(){feeding=true;feedTime=0;paused=false;controls.unlock();pauseOverlay.classList.remove('visible');jumpscare.classList.add('active');showDanger('YOU CAUGHT THEM.',2);}
function updateFeed(delta){feedTime+=delta;const target=human.position.clone().add(new THREE.Vector3(0,1.5,0));camera.lookAt(target);camera.position.lerp(target.clone().add(new THREE.Vector3(0,1.2,1.5)),Math.min(1,delta*2.3));human.scale.multiplyScalar(0.995);if(feedTime>2.5){feeding=false;ended=true;jumpscare.classList.add('finished');setTimeout(()=>endOverlay.classList.add('visible'),300);}}
...
addEventListener('keydown',e=>keys.add(e.code));addEventListener('keyup',e=>keys.delete(e.code));addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);});
